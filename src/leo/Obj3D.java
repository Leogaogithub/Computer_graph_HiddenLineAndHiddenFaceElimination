...
{  private float rho, d, theta=0.30F, phi=1.3F, rhoMin, rhoMax, 
   ...
   // assignment 4
   void changeSunDirection(float sunTheta, float sunPhi, float radius){
	   sunX = radius*Math.sin(sunPhi)*Math.cos(sunTheta); 
	   sunY = radius*Math.sin(sunPhi)*Math.sin(sunTheta); 
	   sunZ = radius*Math.cos(sunPhi);
   }  
}
